...
//   );
// }
// components/voter/vote-form.tsx
"use client";
...
export function VoteForm({ electionId, candidates }: VoteFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Submit the vote - simplified insertion without RLS checks
      const { error } = await supabase.from("votes").insert({
        election_id: electionId,
        voter_id: user.id,
        candidate_id: selectedCandidate,
      ...
      if (error) {
        ...
        // If RLS blocks the insert, show a more specific message
        if (error.code === "42501") {
          toast.error("You do not have permission to vote in this election.");
      ...
      toast.error("Your vote has been successfully recorded.");
